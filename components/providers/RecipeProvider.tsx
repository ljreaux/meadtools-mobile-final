...
export default function RecipeProvider({
  ...
  const changeVolumeUnits = (option: Option) => {
    if (option) {
  ...
  const changeWeightUnits = (option: Option) => {
    if (option) {
      const unit = option.value;
      setRecipeData((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    const fetchIngredients = async () => {
      try {
        const response = await fetch("https://meadtools.com/api/ingredients");
    ...
    const fetchAdditives = async () => {
      try {
        const response = await fetch("https://meadtools.com/api/additives");
        const data = await response.json();
